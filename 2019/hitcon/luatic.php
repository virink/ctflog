...
foreach (Array('_GET', '_POST') as $request) {
	foreach ($$request as $k => $v) {
		${$k} = str_replace(str_split("[]{}=.'\""), "", $v);
	}

}

// if (strlen($token) == 0) {
// 	highlight_file(__FILE__) and exit();
// }
...
// if (strlen($status) == 0 || $status == 'Stopped') {
// 	die('Start Redis first');
// }
...
// 	die('Auth fail');
// }
// var_dump($redis->rawCommand('eval', 'return 0', '0'));
...
if ($redis->get($TEST_KEY) !== $TEST_VALUE) {
	die('Something Wrong? : rawCommand');
}
